...
api.structure = {
  directories: ['src'],
  sub_directories: ['src/constants', 'src/containers', 'src/libraries'],
  others: ['src/containers/employee']
};
...
api.generateFiles = async _answers => {
  ...
  await createFileWithContent(
    `./${_answers.appName}/${constantsJS.path}`,
    constantsJS.filename,
    content
  );

  await createFileWithContent(
    `./${_answers.appName}/${apiJS.path}`,
    apiJS.filename,
    apiJS.content
  );
  await createFileWithContent(
    `./${_answers.appName}/${storeJS.path}`,
    storeJS.filename,
    storeJS.content
  );

  await createFileWithContent(
    `./${_answers.appName}/${employeeIndexJS.path}`,
    employeeIndexJS.filename,
    employeeIndexJS.content
  );
  await createFileWithContent(
    `./${_answers.appName}/${employeeListJS.path}`,
    employeeListJS.filename,
    employeeListJS.content
  );
  await createFileWithContent(
    `./${_answers.appName}/${employeeSingleJS.path}`,
    employeeSingleJS.filename,
    employeeSingleJS.content
  );
};
